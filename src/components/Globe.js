...
function resolveCollisions(items, boxWidth = 190, defaultHeight = 44, canvasSize = 600) {
  ...
  const adjusted = items.map((item) => ({ 
    ...item, 
    height: (item.affected && item.affected.length > 0) ? 66 : 44, // Expand box collision height if relations are present at bottom
    adjX: item.x, 
    adjY: item.y 
  }));
...
export default function Globe({ region, newsItems, canvasSize = 760, hoveredCountry, onHoverCountry, onClickCountry, isExportMode = false, draggedOffsets = {}, onDraggedOffsetsChange, monthId }) {
  ...
  useEffect(() => {
    ...
    svg.selectAll('.globe-land')
      .each(function(d) {
        const code = getCountryCode(d.id);
        const isHovered = hoveredCountry !== null && code === hoveredCountry;
